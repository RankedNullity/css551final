...
{
    // Get the time-based offset
    float2 offset = sin(_Time.y * 0.5) * 0.1; // You can adjust the values as needed

    // Apply the offset to the UV coordinates
    float2 offsetUV = i.uv + offset;

    // Calculate the grid color based on the offset UV coordinates
    fixed4 gridColour = (_GridColour * GridTest(offsetUV)) + tex2D(_MainTex, offsetUV);
